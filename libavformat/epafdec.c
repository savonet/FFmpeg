...
static int epaf_read_header(AVFormatContext *s)
{
    ...
    case 0:
        ...
        break;
    case 1:
        st->codec->codec_id = AV_CODEC_ID_PCM_S24PARIS;
        break;
    case 2:
    ...
    if (!(st->codec->bits_per_coded_sample = av_get_bits_per_sample(st->codec->codec_id))) {
        st->codec->block_align = 32 * st->codec->channels;
        st->codec->bit_rate = st->codec->sample_rate * st->codec->channels * 128 / 5;
    } else {
...
static int epaf_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    int ret;

    ret = av_get_packet(s->pb, pkt, 256 * s->streams[0]->codec->block_align);
